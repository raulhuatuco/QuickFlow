...
void QKflow::on_actionSlack_triggered() {
  ...
  if(windowAddSlack->exec() == QDialog::Accepted) {
    if(pnNetwork_->addSlack(slack) == false) {
      delete slack;
      QMessageBox::critical(this, tr("Failed to add Slack Bar"),
                            tr("Only 1 slack bar is allowed."), QMessageBox::Ok);
    }
  } else {
    delete slack;
  }
...
void QKflow::on_actionPQBar_triggered() {
  ...
  if(windowAddPq->exec() == QDialog::Accepted) {
    if(pnNetwork_->addPq(pq) == false) {
      delete pq;
      QMessageBox::critical(this, tr("Failed to add PQ Bar"),
                            tr("Bars must have an unique identifier."),
                            QMessageBox::Ok);
    }
  } else {
    delete pq;
